...
if not args.no_train:
    ...
    for e in range(starting_epoch , params['num_epochs'] ):
        ...
        if (e % params['test_interval']) == 0 and e!=0:
            ...
            check_if_last_epoch = params['num_epochs'] - 1 - e
            if check_if_last_epoch == 0:
                is_last_epoch = True
            else:
                is_last_epoch = False
                
            if is_last_epoch == False:
                test_loss, test_acc = test(gen_test, decolle_loss, net, params['burnin_steps'], is_last_epoch, print_error = True)
            else:
